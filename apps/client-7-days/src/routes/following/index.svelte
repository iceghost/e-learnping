...
<div class="mx-auto mt-5 w-full max-w-sm">
	<div class="mx-2">
		<PageHeading>Danh sách site môn</PageHeading>
		<p class="mt-3 text-justify text-sm text-slate-600">
			Có thể có nhiều site cho một môn. Các site để nhóm lớp <span
				class="font-functional text-sm font-medium tracking-wider text-sky-700">L00</span
			> là site lớp nhỏ, những site để tên giảng viên là site lớp lớn.
		</p>
		<p class="mt-2 text-justify text-sm text-slate-600">
	...
	<div class="sticky top-0 mx-2 bg-white/80 py-2">
		<form on:submit|preventDefault>
			<input
				type="text"
				class="block h-full w-full rounded-full border-2 border-slate-200 bg-white/80 py-2
			pl-8 font-functional text-slate-500 placeholder:text-slate-300 font-medium"
	...
	{#await courses then courses}
		...
		{#each processed as [category, courses]}
			<details open>
				<summary
					class="font-display border-b-2 border-slate-200 bg-slate-50 px-2 py-1 font-semibold text-slate-700"
				>
					{category.name}
				</summary>
				<ul class="space-y-3 px-2 py-5">
		...
		{/each}
		<p class="font-functional mt-10 text-center text-slate-700">
